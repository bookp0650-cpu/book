...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        # addmm_act is an inference-only fused path: it detaches fc1 weights and
        # explicitly rejects autograd. Use the regular Linear + activation path
        # whenever gradients are needed; otherwise keep the fused fast path.
        fc1_needs_grad = (
            torch.is_grad_enabled()
            and (
                x.requires_grad
                or self.fc1.weight.requires_grad
                or (self.fc1.bias is not None and self.fc1.bias.requires_grad)
            )
        )
        if fc1_needs_grad:
            x = self.act(self.fc1(x))
        else:
            # Global grad mode can still be enabled while this particular module
            # is frozen, so enter no_grad explicitly before calling addmm_act.
            with torch.no_grad():
