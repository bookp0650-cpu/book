...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        self.freeze_param_patterns = freeze_param_patterns or []
    ...
    def _setup_components(self):
        ...
        self._freeze_parameters_by_pattern()
    ...
    def _freeze_parameters_by_pattern(self):
        """Freeze parameters that match Unix shell-style name patterns.

        This is a real freeze (requires_grad=False), unlike assigning lr=0,
        which still builds gradients and optimizer state.
        """
        if not self.freeze_param_patterns:
            return

        frozen_names = []
        for name, param in self.model.named_parameters():
            if any(fnmatch.fnmatch(name, pattern) for pattern in self.freeze_param_patterns):
                param.requires_grad_(False)
                frozen_names.append(name)

        if not frozen_names:
            raise ValueError(
                "freeze_param_patterns matched no model parameters: "
                f"{self.freeze_param_patterns}"
            )
        ...
        logging.info(
            "Frozen %d parameter tensors (%s parameters) using patterns: %s",
            len(frozen_names),
            f"{frozen_count:,}",
            self.freeze_param_patterns,
        )
